...
public class GameNpc extends GameEntity {
    ...
    public void setNPCState() {
        if (getM_npc_target_state() != getM_npc_state()) {
            ...
            setM_frames_step(0);//重置动画播放速度步数池
            detectNPCDrawWay();
            if (getM_height_ratio() != 1.0f)
             ...
             if (m_npc_state!= State.NPCState.ATTACK&&m_npc_state!= State.NPCState.DEAD) {
                setM_bias_x(0);
            }
             else if (temp_state==State.NPCState.ATTACK)//如果上个状态是攻击说明下个状态为死亡先进入暂停状态
             {
                 setM_npc_target_state(State.NPCState.STAND);
                 setNPCState();
             }
            else if (getM_towards()>0) {
                setM_bias_x(-(((getM_width() / getM_frames() - 2 * getM_width_gap()) - (temp_width / temp_frames - 2 * temp_gap)) / 2));//参照人物中部
    ...
    public void resetDrawParameter()
    {
        ...
        {//当角色生命值归零
            ...
            else setM_index((getM_index() % getM_frames()) + getM_frames_step());//设置角色死亡动画下一图像
        }
        else setM_index((getM_index() % getM_frames()) + getM_frames_step());//设置角色下一图像
    }
    ...
    public void draw(Canvas canvas) {
        if (m_npc_state != State.NPCState.HIDE)
        {
            ...
            m_target_rect = new Rect(getM_x()+getM_bias_x(), getM_y(), getM_x()+getM_bias_x() + getM_width() / getM_frames() - 2 * getM_width_gap(), getM_y() + getM_height());
            getM_healthbar().setM_x(getM_x()+getM_bias_x()-(getM_healthbar().getM_width()-(getM_width() / getM_frames() - 2 * getM_width_gap()))/2);
            getM_healthbar().setM_y(getM_y()+getM_height()-getM_map_bitmap().get("npc_walk").getHeight()-getM_healthbar().getM_height());
            setM_collison_rect(m_target_rect);
            canvas.save();//保存画板状态
            if (getM_towards() < 0) {
                canvas.scale(-1, 1);
                canvas.translate((float) -(2 *(getM_x()+getM_bias_x()) + getM_width() / getM_frames() - 2 * getM_width_gap()), 0);//将原点左移
            }
            canvas.drawBitmap(getM_image(), m_sourse_rect, m_target_rect, null);
            canvas.restore();//恢复画板状态
            getM_healthbar().draw(canvas);
